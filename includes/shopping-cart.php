                                ...
                                <div class="tf-mini-cart-items">
                                    ...
                                    if (isset($_SESSION['customer_id']) && !empty($_SESSION['customer_id'])) {
                                        ...
                                        $cartQuery = "
                                            SELECT ci.id, ci.quantity, ci.price,
                                                   p.id as product_id, p.name, p.slug, p.main_image, p.gallery_image_1
                                            FROM cart_items ci
                                        ...
                                        if ($result = $conn->query($cartQuery)) {
                                    ...
                                    <?php if (!empty($cartItems)): ?>
                                        <?php foreach ($cartItems as $item): ?>
                                            ...
                                            <div class="tf-mini-cart-item file-delete" data-cart-item-id="<?php echo $item['id']; ?>">
                                                <div class="tf-mini-cart-image">
                                                    <?php $imageFile = $item['main_image'] ?: $item['gallery_image_1']; ?>
                                                    <?php $cartImage = product_image_path($imageFile); ?>
                                                    <img class="lazyload" 
                                                         data-src="<?php echo htmlspecialchars($cartImage); ?>" 
                                                         src="<?php echo htmlspecialchars($cartImage); ?>" 
                                                ...
                                                <div class="tf-mini-cart-info flex-grow-1">
                                                    <div class="mb_12 d-flex align-items-center justify-content-between flex-wrap gap-12">
                                                        <div class="text-title"><a href="product-detail.php?slug=<?php echo urlencode($item['slug']); ?>" class="link text-line-clamp-1"><?php echo htmlspecialchars($item['name']); ?></a></div>
                                                        <div class="text-button tf-btn-remove remove" onclick="removeFromCart(<?php echo $item['id']; ?>)">Remove</div>
                                                    </div>
                                                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-12">
                                                        <div class="text-secondary-2">Qty: <?php echo intval($item['quantity']); ?></div>
                                                        <div class="text-button">Price: ₹<?php echo number_format($item['price'], 2); ?></div>
                                                    </div>
                                                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-12">
                                                        <div class="text-secondary-2">Subtotal</div>
                                                        <div class="text-title">₹<?php echo number_format($itemTotal, 2); ?></div>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
                        ...
                        <!-- <div class="tf-mini-cart-bottom">
                            ...
                            <div class="tf-mini-cart-bottom-wrap">
                                <div class="tf-cart-totals-discounts">
                                    <h5>Subtotal</h5>
                                    <h5 class="tf-totals-total-value">₹<?php echo number_format($totalPrice, 2); ?></h5>
                                </div>
                                <div class="tf-mini-cart-view-checkout">
                                    <a href="cart.php" class="tf-btn w-100 btn-white radius-4 has-border"><span class="text">View cart</span></a>
                                    <a href="cart.php" class="tf-btn w-100 btn-fill radius-4"><span class="text">Check Out</span></a>
                                </div>
...
function removeFromCart(cartItemId) {
    if (confirm('Are you sure you want to remove this item?')) {
        ...
        .then(data => {
            if (data.success) {
                // Reload cart items by reloading the modal
                location.reload();
            } else {
